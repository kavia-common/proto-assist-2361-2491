...
if bash "${SCRIPT_DIR}/startup.sh"; then
  echo "[Database] [EXIT] startup.sh completed (process ended)."
else
  echo "[Database] [WARN] startup.sh returned non-zero; continuing with fallbacks."
fi
...
if command -v node >/dev/null 2>&1 && [ -f "${SCRIPT_DIR}/index.js" ]; then
  echo "[Database] [FALLBACK] starting Node health server via index.js on port ${DB_HEALTH_PORT} ..."
  exec node "${SCRIPT_DIR}/index.js"
fi

# Final fallback: keep container alive; try netcat bind or tail
echo "[Database] [FALLBACK] Node not available or index.js missing; entering keep-alive mode on port ${DB_HEALTH_PORT} if possible ..."
if command -v nc >/dev/null 2>&1; then
  ...
  echo "[Database] [FALLBACK] netcat not available; tail -f /dev/null as final no-op fallback."
  tail -f /dev/null
fi
